...
def edge_list_to_index(edge_list):
    edge_index = []
    for idx, hedge in enumerate(edge_list):
        for node in hedge:
...
def load_extra_dataset(path='../hyperGCN/data/', dataset = 'cora', train_percent = 0.025):
    '''
    this will read the citation dataset from HyperGCN, and convert it edge_list to 
    [[ -V- | -E- ]
     [ -E- | -V- ]]
    ...
    if dataset == 'twitch':
        ...
        labels = torch.LongTensor(clean_labels)
    else:
        labels = torch.LongTensor(labels)
